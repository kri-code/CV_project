...
class FusionNetwork(nn.Module):
    ...
    def __init__(self, text_dim, audio_dim):
        ...
        self.fusionlayer = FusionLayerMLP(text_dim, audio_dim)

    def forward(self, text_data, audio_data):
        text_embedding = self.bert.embedding(text_data)

        pred = self.fusionlayer(text_embedding, audio_data)
...
class FusionModelPipeline:
    ...
    def train(self):
        ...
        for epoch in (pbar := tqdm.tqdm(range(self.num_epochs))):
            ...
            total = 0
            self.model.train()
            for i, (labels, text, audio) in enumerate(self.train_dataloader):
            ...
            if epoch % 5 == 0:
                self.save(f"fusion/ParameterRealAU_{epoch}.pt")
            ...
            dev_total = 0
            if epoch % 2 == 0:
                for i, (labels, text, audio) in enumerate(self.dev_dataloader):
                    audio = audio.to(self.device)
                    #text = text.to(self.device)
                    labels = labels.to(self.device)
                    outputs = self.model(text, audio)
                ...
                df2.to_csv("fusion/dev_accuraciesAU1.csv", index=False)
                df = pd.DataFrame({"Loss": losses})
                df.to_csv("fusion/lossesAU1.csv", index=False)
...
if __name__ == "__main__":
    ...
    model = FusionNetwork(text_dim, audio_dim)
