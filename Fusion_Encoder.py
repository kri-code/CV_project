...
from torch import nn, flatten, load
...
class video_encoder(nn.Module):
    def __init__(self):
        ...
        self.Conv3D_e11    = nn.Conv3d(      1     , 2, 3, padding=1)
        self.Conv3D_e12    = nn.Conv3d(   2, 2, 3, padding=1)
        self.MaxPool3D_e1  = nn.MaxPool3d(2)

        self.Conv3D_e21    = nn.Conv3d(2, 4, 3, padding=1)
        self.Conv3D_e22    = nn.Conv3d(4, 4, 3, padding=1)
        self.MaxPool3D_e2  = nn.MaxPool3d(2)

        self.Conv3D_e31    = nn.Conv3d(4, 8, 3, padding=1)
        self.Conv3D_e32    = nn.Conv3d(8, 8, 3, padding=1)
        self.MaxPool3D_e3  = nn.MaxPool3d(2)

        self.Conv3D_b1     = nn.Conv3d(8, 8, 3, padding=1)


    def forward(self, X):

        X = f.relu(self.Conv3D_e11(X))
        X = self.Dropout(X)
        X = f.relu(self.Conv3D_e12(X))

        X = self.MaxPool3D_e1(X)

        X = f.relu(self.Conv3D_e21(X))
        X = self.Dropout(X)
        X = f.relu(self.Conv3D_e22(X))

        X = self.MaxPool3D_e2(X)

        X = f.relu(self.Conv3D_e31(X))
        X = self.Dropout(X)
        X = f.relu(self.Conv3D_e32(X))

        X = self.MaxPool3D_e3(X)

        X = f.relu(self.Conv3D_b1(X))
        return flatten(X,1)
    ...
    def load_weights(self, path):
        ...
        for i in range(14):
            key = list(checkpoint["state_dict"].keys())[i]
...
class FusionLayerMLP(nn.Module):
    ...
    def forward(self, text_embedding, audio_embedding, video_embedding):
        ...
        video_embedding = self.encoderlin(video_embedding)
...
class FusionNetwork(nn.Module):
    ...
    def __init__(self, text_dim, video_dim, audio_dim):
        ...
        self.encoder.load_weights("fusion/autoencoder2_weights.pth")
    ...
    def forward(self, text_data, video_data, audio_data):
        video_embedding = self.encoder(video_data)

        text_embedding = self.bert.embedding(text_data)
        pred = self.fusionlayer(text_embedding,  audio_data, video_embedding)
...
class FusionModelPipeline:
    ...
    def train(self):
        ...
        for epoch in (pbar := tqdm.tqdm(range(self.num_epochs))):
            ...
            if epoch % 5 == 0:
                self.save(f"fusion/ParameterEncoder{epoch}.pt")
            ...
            if epoch % 2 == 0:
                ...
                df2.to_csv("fusion/dev_accuraciesEncoder.csv", index=False)
                df = pd.DataFrame({"Loss": losses})
                df.to_csv("fusion/lossesEncoder.csv", index=False)
    ...
    def test(self):
        with torch.no_grad():
            correct = 0
            total = 0
            for i, (video, labels, text, audio) in enumerate(self.test_dataloader):
                video = video.to(self.device)
                audio = audio.to(self.device)
                labels = labels.to(self.device)
                outputs = self.model(text, video, audio)
